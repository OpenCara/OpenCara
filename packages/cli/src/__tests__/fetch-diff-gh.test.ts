/**
 * Tests for the gh/HTTP diff-fetch path in commands/agent.ts.
 *
 * Covers:
 * - `fetchDiffViaGh` — the gh CLI tier (success, ENOENT, non-zero exit,
 *   HTTP error wrapping, and abort-signal handling).
 * - `build404Hint` — the 404 hint text used by the HTTP fallback, which
 *   branches on whether gh is authenticated so we stop blaming auth when
 *   auth is fine.
 */
...
import { fetchDiffViaGh, build404Hint } from '../commands/agent.js';
