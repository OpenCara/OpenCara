...
- #775 [cli-dev, priority:medium, bug] CLI silently falls back to `gh pr diff` when `task.base_ref` is missing, even with worktree available — **IN REVIEW** (PR #780 opened 2026-04-22; scope expanded 2026-04-22 per team-lead: worktree-present MUST use git-diff only, no gh fallback, failure propagates to task rejection. cli-dev-775 amending PR #780. Issue comment 4297952027 has the new acceptance criterion.) Observed on `ParadiseEngine/ParadiseEngine#38` 07:57 — one agent hit gh path while two others on same PR got local git-diff. Fix: (A) emit `logWarn` on the falsy-guard skip, (B) derive merge-base locally via `git symbolic-ref refs/remotes/origin/HEAD` + `git merge-base`, (C) **new**: with worktree present, git-diff is the ONLY diff source; failure → safeReject, never → gh fetch. gh-fetch remains only when worktree checkout itself failed.
