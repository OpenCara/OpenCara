...
- #783 [server-dev, P2, bug, M] `POST /api/tasks/:id/heartbeat` endpoint + per-claim `last_heartbeat_at` column — **DISPATCHED 2026-04-23** (Agent=server-dev backfilled, moved to In progress). Adds D1 migration, updates `reclaimAbandonedClaims` to use claim-level heartbeat when present, agent-level fallback keeps back-compat for old CLIs/pre-migration rows. Paired with #782.
