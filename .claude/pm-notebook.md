...
- #776 [server-dev, priority:medium, bug] Audit: why are some review tasks created without `base_ref`? — **IN REVIEW** (Phase 1 audit posted 2026-04-22 — all 5 PR-scoped paths correctly populate base_ref; CLI symptom mechanism confirmed at `tasks.ts:997` empty→undefined coercion; ParadiseEngine/#38 NOT reproducible from main. Phase 2 PR #781 opened `[server-dev] enforce base_ref invariant on PR-scoped task inserts`, CI green, hybrid (c) log-and-allow guard per team-lead decision; moved In-progress→In-review.)
