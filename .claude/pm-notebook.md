...
- #767 [cli-dev, priority:low] Improve 404 diff-fetch hint — don't blame auth when user is authenticated — **DONE** (PR #778 merged 2026-04-22 commit 00140c7 — bot review approved on first pass. Board → Done, issue closed by PM.) Scope refreshed 2026-04-22 — post-#726 the HTTP 404 path is a fallback-of-a-fallback; auth-branching fix is still valid but narrower than the original spec.

### Batch complete (2026-04-22) — 5 issues dispatched-to-merged same day

All 5 issues dispatched by team-lead promotion Backlog→Ready → PM backfilled Agent field + moved to In progress → team-lead spawned 5 worktree dev agents (no server-dispatched implement claimed) → PRs opened → bot review + fix cycle → all merged:

| #                      | PR   | Merge commit | Bot round                                                               |
| ---------------------- | ---- | ------------ | ----------------------------------------------------------------------- |
| #767 404 hint          | #778 | 00140c7      | approved first pass                                                     |
| #774 flaky test        | #777 | e2f98eb      | approved first pass                                                     |
| #773 event prune       | #779 | fb6359b      | 1 fix cycle (Node cron path gap)                                        |
| #776 base_ref audit    | #781 | a910a50      | 1 fix cycle (bot reviewed pre-flip; agent rebutted, team-lead accepted) |
| #775 base_ref fallback | #780 | f363eb6      | 1 fix cycle (scope expansion: worktree→git-diff only)                   |

All 5 issues closed + board → Done. 5 worktrees pending team-lead cleanup.

### Proactive finding (2026-04-22, PM idle review)

- #773 [server-dev, priority:low, enhancement] Prune unbounded `agent_reliability_events` (and optionally `reputation_events`) rows in scheduled cleanup — **DONE** (PR #779 merged 2026-04-22T16:38:42Z commit fb6359b, bot review resolved via fix commit 24e55a5; `Closes #773` keyword auto-closed issue on merge; board → Done; pointer comment added by PM.) Fix: `runScheduledEventPrunes` extracted to run for both CF Workers `scheduled()` and Node self-hosted cron paths.
- #774 [cli-dev, priority:low, enhancement] Flaky test: `cli-server-integration 'tool crash reports error to server, slot is freed'` hits 15s timeout under load — **DONE** (PR #777 merged 2026-04-22 commit e2f98eb — bot review approved on first pass. Board → Done, issue closed by PM.) Typical ~13–14s vs 15s deadline leaves ~1s headroom. Fix: added `{ timeout: 30000 }` to the `it(...)`.

### Bug: base_ref silent gh-fallback (team-lead reported, 2026-04-22)
...
- #785 [server-dev, priority:high, bug] "Strictly forbid duplicate-model reviews within a task group" — **IN REVIEW 2026-04-24** (PR #786 opened by worktree agent server-dev-785). Board → In review. Root cause: `isModelDiversityVisible` (`packages/server/src/routes/tasks.ts:130-146`) returns true once `Date.now() - claimedAt >= modelDiversityGraceMs` — grace-period escape lets the second instance in. Fix: strict worker-level diversity (409 CLAIM_CONFLICT when the model already has a non-terminal claim in the group), terminal claims do not lock out retries, summary claims remain independent. Live evidence: ParadiseEngine/ParadiseEngine#55 bot-review listed `claude-opus-4-7/claude` twice (two Opus instances each claimed a worker slot on same group). Dev agent handles review/fix loop; PM/team-lead merges when clean.
...
- #782 [cli-dev, P2, bug, M] Periodic heartbeat during tool execution to prevent mid-run claim reclaim — **IN REVIEW 2026-04-24** (PR #788 opened by worktree agent cli-dev-782; board → In review). Live-agent evidence: 13-min Codex review on ParadiseEngine#52 (task 4c05ea84) submitted 409 "Claim already error" because `reclaimAbandonedClaims(10min)` flipped the active claim to error mid-run. CLI only heartbeats on claim-create + result-submit; nothing during `toolExecutor.run`. Paired with #783.
- #783 [server-dev, P2, bug, M] `POST /api/tasks/:id/heartbeat` endpoint + per-claim `last_heartbeat_at` column — **DONE 2026-04-24T09:35:14Z** (PR #787 merged commit 67177bb by worktree agent server-dev-783; board → Done, issue closed by PM). Adds D1 migration, updates `reclaimAbandonedClaims` to use claim-level heartbeat when present, agent-level fallback keeps back-compat for old CLIs/pre-migration rows. Paired with #782.
...
- PR #780 [cli-dev-775] merged 2026-04-22 commit f363eb6 — [cli-dev] Derive default branch locally when task.base_ref is missing (#775 DONE, fix cycle round 2, final design: worktree-present → git-diff only, no gh fallback; fix commit b14561c)
- PR #779 [server-dev-773] merged 2026-04-22T16:38:42Z commit fb6359b — [server-dev] Prune unbounded reliability and reputation events in scheduled hook (#773 DONE, fix cycle round 2, `runScheduledEventPrunes` extracted to cover both Workers scheduled() and Node self-hosted cron)
