...
- #775 [cli-dev, priority:medium, bug] CLI silently falls back to `gh pr diff` when `task.base_ref` is missing, even with worktree available — **BACKLOG**. Observed on `ParadiseEngine/ParadiseEngine#38` 07:57 — one agent hit gh path while two others on same PR got local git-diff. Root cause: `agent.ts:751` guard `if (taskCheckoutPath && taskBareRepoPath && base_ref)` is silent when only `base_ref` is falsy (other two skip branches already warn). Fix: (A) emit `logWarn` on the falsy-guard skip, (B) make `diffFromWorktree` derive merge-base locally via `git symbolic-ref refs/remotes/origin/HEAD` + `git merge-base` so it works without caller-provided `base_ref`. Relax the guard to require only worktree paths.
- #776 [server-dev, priority:medium, bug] Audit: why are some review tasks created without `base_ref`? — **BACKLOG**. Companion to #775 — addresses the root cause. Every server path that INSERTs a review task (webhook.ts createPrTaskGroups, /opencara review comment handler, future /api/reviews/trigger #770, re-queue/retry path, summary-task creation) must populate `base_ref` from `pr.base.ref`/`pr.base.sha`. Gap-report deliverable posted in-issue. Parallel with #775, not blocking.
