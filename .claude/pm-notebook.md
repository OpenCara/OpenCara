...
- #767 [cli-dev, priority:low] Improve 404 diff-fetch hint — don't blame auth when user is authenticated — **IN REVIEW** (PR #778 opened 2026-04-22 `[cli-dev] Branch 404 diff-fetch hint on gh auth state`, CI green; moved In-progress→In-review). Scope refreshed 2026-04-22 — post-#726 the HTTP 404 path is a fallback-of-a-fallback; auth-branching fix is still valid but narrower than the original spec. See issue comments 4294957609 + 4295071228 for updated acceptance criteria.

### Proactive finding (2026-04-22, PM idle review)

- #773 [server-dev, priority:low, enhancement] Prune unbounded `agent_reliability_events` (and optionally `reputation_events`) rows in scheduled cleanup — **IN REVIEW** (PR #779 opened 2026-04-22 `[server-dev] Prune unbounded reliability and reputation events in scheduled hook`, CI green; moved In-progress→In-review). Rooted in PR #724's new table that's append-only with a 30-min query window: rows older than `RELIABILITY_WINDOW_MS` are never read, never deleted. Same latent issue on `reputation_events` (14-day half-life decay → weight at 180d is ~0.00014). Fix: add `cleanupStaleReliabilityEvents` to the scheduled hook next to existing `cleanupTerminalTasks` / `cleanupExpiredOAuthCache`. Reputation prune is optional follow-up; issue body specifies both as one bundle since the pattern is identical.
- #774 [cli-dev, priority:low, enhancement] Flaky test: `cli-server-integration 'tool crash reports error to server, slot is freed'` hits 15s timeout under load — **IN REVIEW** (PR #777 opened 2026-04-22 `[cli-dev] Raise flaky integration test timeout to 30s`, CI green; moved In-progress→In-review). Observed 2026-04-22 during M23-shelve docs gate (1 failure first run, passed in isolation at 13.6s and full-suite retry 2927/2927). Typical ~13–14s vs 15s deadline leaves ~1s headroom. Recommended fix: add `{ timeout: 30000 }` to the `it(...)` (2-char change, zero risk). Fallback: slim test body. Not a functional bug.

### Bug: base_ref silent gh-fallback (team-lead reported, 2026-04-22)

- #775 [cli-dev, priority:medium, bug] CLI silently falls back to `gh pr diff` when `task.base_ref` is missing, even with worktree available — **IN REVIEW** (PR #780 opened 2026-04-22 `[cli-dev] Derive default branch locally when task.base_ref is missing`, CI green; moved In-progress→In-review). Observed on `ParadiseEngine/ParadiseEngine#38` 07:57 — one agent hit gh path while two others on same PR got local git-diff. Root cause: `agent.ts:751` guard `if (taskCheckoutPath && taskBareRepoPath && base_ref)` is silent when only `base_ref` is falsy (other two skip branches already warn). Fix: (A) emit `logWarn` on the falsy-guard skip, (B) make `diffFromWorktree` derive merge-base locally via `git symbolic-ref refs/remotes/origin/HEAD` + `git merge-base` so it works without caller-provided `base_ref`. Relax the guard to require only worktree paths.
- #776 [server-dev, priority:medium, bug] Audit: why are some review tasks created without `base_ref`? — **IN PROGRESS — PHASE 1 AUDIT DONE, PHASE 2 GATED** (Phase 1 posted 2026-04-22; conclusion: all 5 PR-scoped task-creation paths correctly populate base_ref from webhook payload / fetchPrDetails. CLI symptom mechanism confirmed: `tasks.ts:997` coerces empty-string → undefined in PollTask response. ParadiseEngine/#38 NOT reproducible from current main — likely stale row. Phase 2 proposal: boundary hardening (invariant guard at D1DataStore insert, defensive warns at webhook entry, summary-inheritance assertion, #770 acceptance line, tests). PM awaiting team-lead go/no-go on scope + pick a/b/c on the D1 insert-invariant question: (a) reject, (b) log+allow, (c) hybrid.)
