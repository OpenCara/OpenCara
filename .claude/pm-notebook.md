...
- #773 [server-dev, priority:low, enhancement] Prune unbounded `agent_reliability_events` (and optionally `reputation_events`) rows in scheduled cleanup — **IN REVIEW — FIX CYCLE** (PR #779 bot review requested changes 2026-04-22, server-dev-773 dispatched to fix). Rooted in PR #724's new table that's append-only with a 30-min query window: rows older than `RELIABILITY_WINDOW_MS` are never read, never deleted. Same latent issue on `reputation_events` (14-day half-life decay → weight at 180d is ~0.00014). Fix: add `cleanupStaleReliabilityEvents` to the scheduled hook next to existing `cleanupTerminalTasks` / `cleanupExpiredOAuthCache`.
...
- #775 [cli-dev, priority:medium, bug] CLI silently falls back to `gh pr diff` when `task.base_ref` is missing, even with worktree available — **IN REVIEW — FIX CYCLE** (PR #780 bot review requested changes 2026-04-22, cli-dev-775 dispatched to fix + fold in the scope-expansion amendment from issue comment 4297952027). Fix: (A) emit `logWarn` on the falsy-guard skip, (B) derive merge-base locally, (C) with worktree present, git-diff is the ONLY diff source; failure → safeReject, never → gh fetch.
- #776 [server-dev, priority:medium, bug] Audit: why are some review tasks created without `base_ref`? — **IN REVIEW — FIX CYCLE** (PR #781 hybrid-c rework pushed 2026-04-22, CI green; bot review then requested changes, server-dev-776 dispatched to fix.) Phase 1 audit conclusion: all 5 PR-scoped paths correctly populate base_ref; CLI symptom mechanism confirmed at `tasks.ts:997` empty→undefined coercion; ParadiseEngine/#38 NOT reproducible from main. Phase 2 = boundary hardening with hybrid (c) log-and-allow guard per team-lead decision.
