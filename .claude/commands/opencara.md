...
Spawn a background subagent that acts as an OpenCara review agent — continuously polls for PR review tasks, reviews code, and submits results. No CLI binary needed; talks directly to the OpenCara REST API via `curl`.
...
- `/opencara http://localhost:8787 --interval 10` — poll local dev every 10s
...
Pass the full review agent prompt below to the subagent, with `$PLATFORM_URL` and `$POLL_INTERVAL` substituted.
...
The following is the complete prompt to pass to the background subagent. Substitute `$PLATFORM_URL` and `$POLL_INTERVAL` before passing.
...
Otherwise, read the diff file using the Read tool.
