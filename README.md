...
**Cleanup.** When GitHub fires `pull_request.closed` (merged or not), the orchestrator dispatches `opencara internal worktree remove --key <slug>` for every `worktree_pins` row on that head ref, wiping the checkouts and session dirs, and deletes the rows. A stale-worktree prune (every 6h, default retention 3 days) reclaims checkouts no PR-close will reach — schedule / manual runs, PRs closed while the device was offline. Orphaned worktrees on since-revoked devices need manual cleanup.

CLI side: `opencara internal worktree {create,remove,write-session}`. **Keep paired devices on a current CLI build** — device and orchestrator exchange a host-protocol version in the `hello`/`hello-ack` handshake (`packages/shared/src/host-protocol.ts`), and CLIs predating the ACP cutover can't run dispatched agents at all. `npm exec opencara@latest` on each paired host after upgrading the orchestrator.
