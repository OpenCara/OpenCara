...
- [x] fix(prune): bind the retention cutoff as ISO string ::timestamptz ([#236](https://github.com/OpenCara/OpenCara/pull/236))
...
- [ ] **`scm.pull_request_review` never fires on Azure DevOps**, so the review→fix half of `development-lifecycle` (and the standalone `pr-review-fix` flow) is dead there. Azure DevOps service hooks have no reviewer-vote event, and `git.pullrequest.updated` — which does fire on a vote — carries no marker saying what changed. Reviews post; nothing consumes them.
  ...
  - Half of that dedup now exists: `refinePullRequestAction` compares the source commit against the PR's previous delivery, so an update that moved no code is already demoted to `edited` instead of being served as a push. A vote-only update therefore drives nothing today (rather than the wrong thing); giving it its own mapping is what remains.
